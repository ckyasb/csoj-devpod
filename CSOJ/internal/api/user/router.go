	...
	v1 := r.Group("/api/v1")
	{
		...
		authed.Use(api.AuthMiddleware(cfg.Auth.JWT.Secret, db))
		{
			...
			profile := authed.Group("/user")
			{
				...
				profile.GET("/ssh_keys", h.listUserSSHKeys)
				profile.POST("/ssh_keys", h.addUserSSHKey)
				profile.DELETE("/ssh_keys/:id", h.deleteUserSSHKey)
			}

			devpods := authed.Group("/devpods")
			{
				devpods.GET("/options", h.getDevPodOptions)
				devpods.POST("", h.createDevPod)
				devpods.GET("", h.listDevPods)
				devpods.GET("/:id", h.getDevPod)
				devpods.POST("/:id/stop", h.stopDevPod)
				devpods.POST("/:id/start", h.startDevPod)
				devpods.DELETE("/:id", h.deleteDevPod)
				devpods.GET("/:id/ssh", h.getDevPodSSH)
				devpods.GET("/:id/logs", h.getDevPodLogs)
			}
