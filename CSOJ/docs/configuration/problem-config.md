...
# Optional Slurm-like scheduling defaults for submissions to this problem
scheduling:
  account: "course-a"
  qos: "normal"
  priority: 0
  nice: 0
  hold: false
  begin_time: "2025-10-01T09:00:00+08:00"
  deadline: "2025-10-01T12:00:00+08:00"
  time_limit: 60
  dependencies: ""
  reservation: ""
  nodelist: "gpu[01-04]"
...
### `scheduling`
  ...
  - **Type**: `object`
  - **Required**: No
  - **Description**: Slurm-like defaults copied into each submission for this problem.
      - `account`: (string, optional) Account association.
      - `qos`: (string, optional) QoS name configured in `config.yaml`.
      - `priority`: (integer, optional) Manual priority boost.
      - `nice`: (integer, optional) Priority reduction.
      - `hold`: (boolean, optional) Queue the submission in a held state until an admin releases it.
      - `begin_time`: (string, optional) Earliest time at which the submission may run.
      - `deadline`: (string, optional) Time after which the queued submission fails instead of running.
      - `time_limit`: (integer, optional) Slurm-style job wall time in seconds. It participates in scheduler limit checks and caps total batch execution time by tightening each workflow step timeout to the remaining job time. If omitted, each workflow step uses its own `timeout`.
      - `dependencies`: (string, optional) Comma or space separated dependencies. Supported forms include `afterok:<submission_id>[:<submission_id>...]`, `afterany:<submission_id>[:<submission_id>...]`, `afternotok:<submission_id>[:<submission_id>...]`, `after:<submission_id>[:<submission_id>...]`, `aftercorr:<array_job_id>`, and `singleton`. Dependency IDs may reference arrays, for example `array_id_7` or `array_id_[1-3:2]`; bare array IDs wait for the whole array. `singleton` matches active jobs from the same user by Slurm job name.
      - `reservation`: (string, optional) Reservation name from `scheduler.reservations`.
      - `nodelist`: (string, optional) Requested node list. Supports comma, semicolon, or space separated node names and bracket ranges such as `gpu[01-04]`.
      - `exclude`: (string, optional) Nodes to remove from consideration, using the same node-list syntax as `nodelist`.
      - `constraint`: (string, optional) Required node features, separated by `&`, comma, or spaces.
      - `gres`: (string, optional) Generic resources such as `gpu:1`.
      - `tres`: (string, optional) Additional trackable resources for billing/accounting, such as `license/foo:1`. TRES entries prefixed with `license/` also consume global `scheduler.licenses` while the job is running.
      - `array`: (string, optional) Slurm-like job array spec such as `0-9`, `1,3,5`, `0-20:2`, or `0-9%2` where `%2` limits concurrent running array tasks.
...
### `workflow`
  ...
  - **Description**: Defines the core judging process as an array of steps that are executed sequentially. Each object in the array represents a step with the following fields:
      ...
      - `network`: (boolean) Whether to enable network access for this step's container. Defaults to `false` (network disabled). Docker nodes apply this directly. Kubernetes nodes expose the intent as a Pod annotation and require NetworkPolicy or admission control to enforce isolation.
      ...
      - `mounts`: (array of objects, optional) A list of additional volumes to mount into the container. Each mount object has:
          - `type`: (string, optional) The mount type. Defaults to `bind`. On Kubernetes nodes, `bind` maps to `hostPath`, `tmpfs` maps to memory-backed `emptyDir`, and `volume`/`pvc` maps to a PersistentVolumeClaim named by `source`.
